...
export async function fetchAssignedIssues({ baseUrl, authHeader, fetchImpl }) {
  const jql = 'assignee = currentUser() AND resolution = Unresolved ORDER BY updated DESC';
  const url = `${baseUrl}/rest/api/2/search?jql=${encodeURIComponent(jql)}&fields=summary,status&maxResults=50`;
  const response = await fetchImpl(url, { headers: { Authorization: authHeader } });

  if (!response.ok) {
    return { ok: false, status: response.status, message: describeApiError(response.status) };
  }
